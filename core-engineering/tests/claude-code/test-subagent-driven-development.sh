...
if assert_contains "$output" "completeness\|complete\|missed\|missing\|gap\|sanity" "Checks for gaps/completeness"; then
    : # pass
else
    exit 1
fi
...
if assert_contains "$output" "not trust\|don't trust\|skeptic\|verify.*independently\|suspiciously\|unverified" "Reviewer is skeptical"; then
    : # pass
else
    exit 1
fi

if assert_contains "$output" "read.*code\|inspect.*code\|verify.*code\|review.*code\|check.*code\|examine\|independent" "Reviewer reads code"; then
    : # pass
else
    exit 1
fi
...
if assert_contains "$output" "implementer.*fix\|fix.*issues\|fix.*re-review\|send.*back\|back.*fix" "Implementer fixes issues"; then
    : # pass
else
    exit 1
fi
...
if assert_contains "$output" "provide.*directly\|full.*text\|paste\|include.*prompt\|inline\|embed\|pass.*text\|give.*text" "Provides text directly"; then
    : # pass
else
    exit 1
fi
...
output=$(run_claude "What workflow skills are required before using subagent-driven-development? List any prerequisites or required skills." 60)
...
output=$(run_claude "In subagent-driven-development, is it okay to start implementation directly on the main branch?" 60)
