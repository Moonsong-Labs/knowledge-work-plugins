...
# IMPORTANT: Run from skills directory so local dev skills are available
...
# Disable errexit for analysis - grep returns 1 on no match
set +e

# Find the session transcript
# Session files are in ~/.claude/projects/-<working-dir>/<session-id>.jsonl
WORKING_DIR_ESCAPED=$(cd "$SCRIPT_DIR/../.." && pwd | sed 's/\//-/g')
...
if [ -z "$SESSION_FILE" ]; then
    echo "WARNING: Could not find session transcript in: $SESSION_DIR"
    echo "Falling back to output file analysis..."
    SESSION_FILE=""
fi

ANALYSIS_FILE="${SESSION_FILE:-$OUTPUT_FILE}"
if [ -n "$SESSION_FILE" ]; then
    echo "Analyzing session transcript: $(basename "$SESSION_FILE")"
else
    echo "Analyzing captured output: $OUTPUT_FILE"
fi
echo ""
...
# Test 1: Skill was invoked (check session or output)
echo "Test 1: Skill invoked..."
if grep -qi "subagent.driven.development\|spec.*compliance\|code.*quality.*review" "$ANALYSIS_FILE"; then
    echo "  [PASS] subagent-driven-development skill was used"
else
    echo "  [FAIL] Skill was not used"
    FAILED=$((FAILED + 1))
fi
echo ""
...
if [ -n "$SESSION_FILE" ]; then
    task_count=$(grep -cE '"name":"Task"|"name":"Agent"' "$SESSION_FILE" || echo "0")
else
    # Infer from output: look for evidence of subagent dispatching
    task_count=$(grep -ciE "subagent|dispatching|implementer" "$OUTPUT_FILE" || echo "0")
fi
if [ "$task_count" -ge 2 ]; then
    echo "  [PASS] Subagents dispatched ($task_count references)"
else
    echo "  [WARN] Could not confirm subagent dispatching ($task_count references)"
fi
echo ""

# Test 3: Task tracking was used
echo "Test 3: Task tracking..."
if [ -n "$SESSION_FILE" ]; then
    todo_count=$(grep -cE '"name":"TodoWrite"|"name":"update_plan"' "$SESSION_FILE" || echo "0")
    if [ "$todo_count" -ge 1 ]; then
        echo "  [PASS] Task tracking used ($todo_count time(s))"
    else
        echo "  [WARN] Could not confirm task tracking"
    fi
else
    echo "  [SKIP] Requires session transcript"
fi
echo ""
...
if [ -n "$SESSION_FILE" ]; then
    ...
    echo "  (skipped - no session transcript found)"
fi
echo ""
