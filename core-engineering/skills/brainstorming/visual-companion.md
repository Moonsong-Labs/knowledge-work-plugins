...
**Finding connection info:** The server writes its startup JSON to `$SCREEN_DIR/.server-info`. If you launched the server in the background and didn't capture stdout, read that file to get the URL and port. When using `--project-dir`, check `<project>/.brainstorm/` for the session directory.
...
- Server, frame template, and client helper are all in `scripts/server.js` (single self-contained file, zero dependencies)
