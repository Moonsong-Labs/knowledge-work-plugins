...
for test in "${tests[@]}"; do
    ...
    if [[ "$test" == *"-integration"* ]] && [ "$TIMEOUT" -lt 1800 ]; then
        test_timeout=1800
    fi
    ...
    if [ "$VERBOSE" = true ]; then
        if timeout --foreground "$test_timeout" bash "$test_path"; then
            end_time=$(date +%s)
        ...
        if output=$(timeout --foreground "$test_timeout" bash "$test_path" 2>&1); then
            end_time=$(date +%s)
            duration=$((end_time - start_time))
